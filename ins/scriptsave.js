...
function load_new(offset)
{
    ...
    if (timedout) return;
    timedout = true;
    setTimeout(() => timedout = false, 1000);
...
async function load_img(offset)
{
    ...
    let max;
    if (offset+LIMIT < posts.length) max = offset+LIMIT;
    else max = posts.length;
    for (let i = offset; i < max; i++)
    {
        ...
        let ext = posts[i].ext;
        if (ext == "mp4") ext = "webp";
        images[i].src = `photos/${posts[i].id}.${ext}`;

        const pcard = document.createElement('div');
        pcard.classList.add("pgrid-item");
        pcard.addEventListener('click', () => showFeed(i));
        const pimg = document.createElement('img');
        pimg.src = `photos/${posts[i].id}.${ext}`;
        pimg.alt = "";
        pcard.appendChild(pimg);
/*        if (posts[i].ext == "mp4")
        {
            const vDiv = document.createElement('div');
            vDiv.classList.add('isVid');
            vDiv.innerHTML =
            `
                <svg
                    viewBox="0 0 24 24"
            ...
            `;
            pcard.appendChild(vDiv);
        }*/
        pgrid.appendChild(pcard);

        images[i].onload = function ()
        {
            ratio = images[i].naturalHeight/images[i].naturalWidth;
            document.getElementById(posts[i].id).children[0].style.height = `${cardWidth*ratio}px`;
            images[i] = "loaded";
        }
    }

    await waitForCondition(() => images.slice(0).every(img => img === "loaded"));
    
    grid.style.display = "none";
    grid.style.opacity = 1;

    Array.from(document.querySelectorAll('.pgrid-item')).slice(offset).forEach(item =>
    {
        const rowHeight = 10;
        const gap = 10;
        const img = item.querySelector('img');
        const rowSpan = Math.ceil((img.getBoundingClientRect().height + gap) / (rowHeight + gap));
    ...
    if (window.location.href.split('ins/?')[1].length)
        showFeed(document.getElementsByClassName("card").length-1-parseInt(window.location.href.split('ins/?')[1]));
...
function showFeed(i)
{
    grid.style.display = "grid";
    pgrid.style.display = "none";
    document.getElementsByClassName("card")[i].scrollIntoView({ block: "center" });    
    window.parent.postMessage(
        {
            action: "backFunc",
            func: "showMain"
    ...
    if (navigator.share)
    {
        try
        {
            await navigator.share({
                ...
                text: `Regarde "${posts[i].desc}" sur Krorion Photos`,
...
window.addEventListener('scroll', () =>
{
    ...
    if (!parseInt(scrollTop + windowHeight - docHeight) && !isLoading) load_new(posts.length);
